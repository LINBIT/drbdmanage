...
class LvmThinPoolNg(storcore.StoragePlugin):
    ...
    def __init__(self):
        ...
        """
        super(LvmThinPoolNg, self).__init__()
        self.reconfigure()
    ...
    def reconfigure(self):
        ...
        try:
            ...
            try:
                ...
                logging.warning(
                    "LvmThinPoolNg: Cannot load configuration file '%s'"
                    % (LvmThinPoolNg.LVM_CONFFILE)
                )
            if conf_loaded is None:
                self._conf = LvmThinPoolNg.CONF_DEFAULTS
            else:
                self._conf = cf.ConfFile.conf_defaults_merge(
                    LvmThinPoolNg.CONF_DEFAULTS, conf_loaded
                )

            # Setup cached settings
            self._vg_path = utils.build_path(
                self._conf[LvmThinPoolNg.KEY_DEV_PATH],
                self._conf[LvmThinPoolNg.KEY_VG_NAME]
            ) + "/"
            self._cmd_create = utils.build_path(
                self._conf[LvmThinPoolNg.KEY_LVM_PATH],
                LvmThinPoolNg.LVM_CREATE
            )
            self._cmd_remove = utils.build_path(
                self._conf[LvmThinPoolNg.KEY_LVM_PATH],
                LvmThinPoolNg.LVM_REMOVE
            )
            self._cmd_lvchange = utils.build_path(
                self._conf[LvmThinPoolNg.KEY_LVM_PATH],
                LvmThinPoolNg.LVM_LV_CHANGE
            )
            self._cmd_vgchange = utils.build_path(
                self._conf[LvmThinPoolNg.KEY_LVM_PATH],
                LvmThinPoolNg.LVM_VG_CHANGE
            )
            self._cmd_lvs    = utils.build_path(
                self._conf[LvmThinPoolNg.KEY_LVM_PATH], LvmThinPoolNg.LVM_LVS
            )
            self._cmd_vgs    = utils.build_path(
                self._conf[LvmThinPoolNg.KEY_LVM_PATH], LvmThinPoolNg.LVM_VGS
            )
            ...
            logging.warning(
                "LvmThinPoolNg plugin: Cannot load state file '%s'"
                % (LvmThinPoolNg.LVM_STATEFILE)
            )
            raise pers_exc
        except Exception as unhandled_exc:
            logging.error(
                "LvmThinPoolNg: initialization failed, unhandled exception: %s"
                % (str(unhandled_exc))
            )
            # Re-raise
            raise unhandled_exc
    ...
    def create_blockdevice(self, name, vol_id, size):
        ...
        try:
            ...
            pool_ratio = LvmThinPoolNg.DEFAULT_POOL_RATIO
            try:
                pool_ratio = float(self._conf[LvmThinPoolNg.KEY_POOL_RATIO])
            ...
            if not pool_exists:
                tries = 0
                while pool is None and tries < LvmThinPoolNg.MAX_RETRIES:
                    if tries > 0:
                        try:
                            time.sleep(LvmThinPoolNg.RETRY_DELAY)
                        except OSError:
                            pass
                    tries += 1
                    ...
                    if pool_exists:
                        ...
                        logging.warning(
                            "LvmThinPoolNg: Attempt %d of %d: "
                            "Creation of pool '%s' failed."
                            % (tries, LvmThinPoolNg.MAX_RETRIES, pool_name)
                        )

                if pool_exists:
                    tries = 0
                    while (blockdev is None and
                           tries < LvmThinPoolNg.MAX_RETRIES):
                        if tries > 0:
                            try:
                                time.sleep(LvmThinPoolNg.RETRY_DELAY)
                            except OSError:
                        ...
                        if lv_exists:
                            ...
                            logging.warning(
                                ...
                                % (tries, LvmThinPoolNg.MAX_RETRIES, lv_name)
                            )
            else:
                logging.error(
                    "LvmThinPoolNg: Creation of pool '%s' failed, "
            ...
            logging.error(
                "LvmThinPoolNg: LV '%s' exists already, but is unknown to "
                "drbdmanage's storage subsystem. Aborting."
                % (lv_name)
            )
        except Exception as unhandled_exc:
            logging.error(
                "LvmThinPoolNg: Block device creation failed, "
        ...
        try:
            ...
            if self._volumes.get(lv_name) is not None:
                ...
                try:
                    ...
                    logging.warning(
                        "LvmThinPoolNg: Cannot find the thin pool "
                        "for LV '%s'"
                        % (lv_name)
                    )
                ...
                tries = 0
                lv_exists = self._check_lv_exists(lv_name)
                while lv_exists and tries < LvmThinPoolNg.MAX_RETRIES:
                    if tries > 0:
                        try:
                            time.sleep(LvmThinPoolNg.RETRY_DELAY)
                        except OSError:
                    ...
                    if pool is not None:
                        ...
                        if pool.is_empty():
                            ...
                            while (pool_exists and
                                   pool_tries < LvmThinPoolNg.MAX_RETRIES):
                                if pool_tries > 0:
                                    try:
                                        time.sleep(
                                            LvmThinPoolNg.RETRY_DELAY
                                        )
                                    except OSError:
                                        pass
                                pool_tries += 1
            ...
            # Collision with a volume not managed by drbdmanage
            logging.error(
                "LvmThinPoolNg: LV '%s' exists, but is unknown to "
            ...
            logging.error(
                "LvmThinPoolNg: Removal of a block device failed, "
                "unhandled exception: %s"
                % (str(unhandled_exc))
            )
    ...
    def create_snapshot(self, name, vol_id, source_blockdev):
        ...
        try:
            ...
            logging.error(
                "LvmThinPoolNg: Creation of snapshot '%s' of LV '%s' failed: "
        ...
        try:
            if pool is not None:
                snaps_base_name = self._lv_name(lv_name, vol_id)
                snaps_suffix    = pool.extract_pool_name_suffix()
                snaps_name      = snaps_base_name + snaps_suffix

                tries = 0
                while blockdev is None and tries < LvmThinPoolNg.MAX_RETRIES:
                    if tries > 0:
                        try:
                            time.sleep(LvmThinPoolNg.RETRY_DELAY)
                        except OSError:
    ...
    def up_blockdevice(self, blockdevice):
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    "LvmThinPoolNg: Volume group activation failed, "
            ...
            if pool_name is not None:
                try:
                    ...
                    logging.error(
                        "LvmThinPoolNg: Thin pool activation failed, "
                        "unable to run external program '%s', error message "
                        "from the OS: %s"
                ...
                logging.error(
                    "LvmThinPoolNg: Incomplete activation of volume '%s', "
                    "cannot find the associated thin pool"
                    % (lv_name)
                )
            try:
                ...
                logging.error(
                    "LvmThinPoolNg: LV activation failed, unable to run "
    ...
    def update_pool(self, node):
        ...
        try:
            ...
            logging.error(
                "LvmThinPoolNg: Retrieving storage pool information failed, "
                "unhandled exception: %s"
                % (str(unhandled_exc))
            )
        finally:
    ...
    def _load_state(self):
        ...
        state_file     = None
        try:
            state_file  = open(LvmThinPoolNg.LVM_STATEFILE, "r")
            ...
            if stored_hash is not None:
                ...
                if computed_hash != stored_hash:
                    logging.warning(
                        "LvmThinPoolNg: Data in state file '%s' has "
                        "an invalid signature, this file may be corrupt"
                        % (LvmThinPoolNg.LVM_STATEFILE)
                    )
            else:
                logging.warning(
                    "LvmThinPoolNg: Data in state file '%s' is unsigned"
                    % (LvmThinPoolNg.LVM_STATEFILE)
                )
            ...
            if io_err.errno == errno.ENOENT:
                ...
                logging.error(
                    "LvmThinPoolNg: Loading the state file '%s' failed "
                    "due to an I/O error, error message from the OS: %s"
                    % (LvmThinPoolNg.LVM_STATEFILE, io_err.strerror)
                )
                raise exc.PersistenceException
        except OSError as os_err:
            logging.error(
                ...
                % (LvmThinPoolNg.LVM_STATEFILE, str(os_err))
            )
            raise exc.PersistenceException
        except Exception as unhandled_exc:
            logging.error(
                "LvmThinPoolNg: Loading the state file '%s' failed, "
                "unhandled exception: %s"
                % (LvmThinPoolNg.LVM_STATEFILE, str(unhandled_exc))
            )
            raise exc.PersistenceException
        finally:
    ...
    def _save_state(self, save_pools, save_volumes):
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    "LvmThinPoolNg: Saving to the state file '%s' failed "
                    "due to an I/O error, error message from the OS: %s"
                    % (LvmThinPoolNg.LVM_STATEFILE, str(io_err))
                )
                raise exc.PersistenceException
            except OSError as os_err:
                logging.error(
    ...
    def _load_conf(self):
        ...
        try:
            conf_file = open(LvmThinPoolNg.LVM_CONFFILE, "r")
            conf_obj  = cf.ConfFile(conf_file)
            loaded_conf = conf_obj.get_conf()
        except IOError as io_err:
            if io_err.errno == errno.EACCES:
                logging.error(
                    "LvmThinPoolNg: Cannot open configuration file '%s': "
                    "Permission denied"
                    % (LvmThinPoolNg.LVM_CONFFILE)
                )
                ...
                logging.error(
                    "LvmThinPoolNg: Cannot open configuration file '%s', "
                    "error message from the OS: %s"
                    % (LvmThinPoolNg.LVM_CONFFILE, io_err.strerror)
                )
        finally:
        ...
        try:
            lvm_proc = subprocess.Popen(
                [
                    ...
                    self._conf[LvmThinPoolNg.KEY_VG_NAME] + "/" + lv_name
                ],
                0, self._cmd_lvs,
                env=self._subproc_env, stdout=subprocess.PIPE,
                close_fds=True
            ...
            # LV was not found
            if lvm_rc != 0 and lvm_rc != LvmThinPoolNg.LVM_LVS_ENOENT:
                raise LvmNgCheckFailedException
        except OSError:
            logging.error(
                "LvmThinPoolNg: Unable to retrieve the list of existing LVs"
            )
            raise LvmNgCheckFailedException
    ...
    def _create_lv(self, lv_name, pool_name, size):
        ...
        try:
            ...
            logging.error(
                "LvmThinPoolNg: ThinPool creation failed, unable to run "
                "external program '%s', error message from the OS: %s"
    ...
    def _create_snapshot(self, snaps_name, lv_name):
        ...
        try:
            ...
            logging.error(
                "LvmThinPoolNg: Snapshot creation failed, unable to run "
                "external program '%s', error message from the OS: %s"
                % (self._cmd_create, str(os_err))
            )
            raise LvmNgException
    ...
    def _create_pool(self, pool_name, size):
        ...
        try:
            ...
            logging.error(
                "LvmThinPoolNg: LV creation failed, unable to run "
                "external program '%s', error message from the OS: %s"
                % (self._cmd_create, str(os_err))
            )
            raise LvmNgException
    ...
    def _remove_lv(self, lv_name):
        ...
        try:
            ...
            logging.error(
                "LvmThinPoolNg: LV removal failed, unable to run "
                "external program '%s', error message from the OS: %s"
