...
class DrbdAdmConf(object):
    ...
    def _get_meta_props(self, item):
        """
        Returns a meta data string that can be written to a resource file
        """
        ret = "#dm-meta:"
        opts = {}
        try:
            props = item.get_props()
            if props:
                opts = props.get_selected_props([consts.CREATEDATE])
        except:
            pass
...
# and use that in all the functions below.
    def write(self, stream, assignment, undeployed_flag, globalstream=False):
        try:
            ...
            stream.write(self._get_meta_props(resource) + '\n\n')
